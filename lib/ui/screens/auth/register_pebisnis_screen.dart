...
class RegisterPebisnisScreen extends StatefulWidget {
  final bool isGoogleRegister;
  final String? initialEmail;
  final String? initialName;

  const RegisterPebisnisScreen({
    super.key,
  ...
  @override
  State<RegisterPebisnisScreen> createState() => _RegisterPebisnisScreenState();
}

class _RegisterPebisnisScreenState extends State<RegisterPebisnisScreen> {
  ...
  void initState() {
    ...
    _businessNameController = TextEditingController(text: widget.initialName ?? '');
  ...
  void _verifyGoogleAccount() {
    ...
    if (existingUser != null && existingUser.role != UserRole.pebisnis) {
      showDialog(
        context: context,
  ...
  Future<void> _handleRegister() async {
    ...
    final bType = _businessTypeController.text.trim();
    ...
    if (bName.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Harap masukkan Nama Bisnis / Perusahaan / Restoran Anda')),
      );
      return;
    }
    ...
    if (bType.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Harap tentukan Jenis Bisnis (misal: Restoran, Hotel, Supermarket, Katering)')),
      );
      return;
    }
    ...
    if (location.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Harap tentukan Letak / Lokasi Bisnis (Gunakan tombol Pilih di Peta)')),
      );
      return;
    }
    ...
    if (phone.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Harap masukkan Nomor Telepon / WhatsApp Bisnis')),
      );
      return;
    }
    ...
    if (doc == null || doc.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Harap upload NIB / Dokumen Izin Usaha untuk melengkapi pendaftaran.')),
      );
      return;
    }
    ...
    final error = await appState.registerPebisnis(
      businessName: bName,
      businessType: bType,
      phone: phone,
      email: email,
    ...
    if (mounted) {
      appState.updateProfile(farmLocation: location, businessType: bType);
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(isGoogle
                ? 'Pendaftaran Google Berhasil! Selamat datang di AgriSync, $bName.'
                : 'Akun Bisnis berhasil didaftarkan! Link verifikasi telah dikirim ke $email. Silakan periksa inbox Gmail Anda.'),
            backgroundColor: AgriColors.primaryGreen,
      ...
      body: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            Text(
              'Beli langsung komoditas pertanian segar berkualitas.',
              style: GoogleFonts.plusJakartaSans(
                fontSize: 12.5,
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'Masukkan alamat email Google untuk verifikasi instan:',
                        style: GoogleFonts.plusJakartaSans(
                          fontSize: 11.5,
                          color: AgriColors.textMuted,
                        ),
            ...
            // Field: Nama Bisnis (Masih lanjut di halaman yang sama)
            AgriTextField(
              controller: _businessNameController,
              hintText: 'Nama Bisnis / Perusahaan / Restoran',
            ),
            const SizedBox(height: 16),

            // Field: Email (Jika email biasa, input teks biasa. Jika google, read-only confirmed)
            if (!isGoogle) ...[
              AgriTextField(
                controller: _emailController,
                hintText: 'Email Bisnis (misal: resto@gmail.com)',
                keyboardType: TextInputType.emailAddress,
              ),
              const SizedBox(height: 16),
            ...
            // Field: Jenis Bisnis (Restoran/Distributor/Hotel/dll)
            AgriTextField(
              controller: _businessTypeController,
              hintText: 'Jenis Bisnis (Restoran, Hotel, Supermarket, dll.)',
            ),
            const SizedBox(height: 16),

            // Field: Lokasi / Alamat Bisnis (dengan OpenStreetMap Picker)
            AgriTextField(
              controller: _locationController,
              hintText: 'Letak / Lokasi Bisnis (Pilih di Peta)',
              suffixIcon: IconButton(
                icon: const Icon(Icons.map_rounded, color: AgriColors.darkOliveBtn),
                tooltip: 'Pilih di Peta (OpenStreetMap)',
                onPressed: () async {
                  final LocationResult? result = await Navigator.of(context).push(
                    MaterialPageRoute(
                      builder: (_) => MapLocationPickerScreen(
                        title: 'Pilih Lokasi Alamat Bisnis',
            ...
            AgriTextField(
              ...
              hintText: 'Nomor Telepon / WhatsApp Bisnis',
              keyboardType: TextInputType.phone,
            ),
            const SizedBox(height: 16),

            // Field: Upload Izin Usaha / NIB
            AgriFileUploadField(
              label: 'Upload NIB / Izin Usaha / Dokumen',
              fileName: _uploadedDocName,
              onTap: () {
                setState(() {
                  _uploadedDocName = 'NIB_Izin_${_businessNameController.text.isNotEmpty ? _businessNameController.text.split(" ").first : "Bisnis"}.pdf (Terlampir)';
                });
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Dokumen izin usaha berhasil dipilih!'),
                    backgroundColor: AgriColors.primaryGreen,
                  ),
                );
              },
            ...
            // Button Submit
            AgriPillButton(
              text: isGoogle ? 'DAFTARKAN AKUN GOOGLE PEBISNIS' : 'BUAT AKUN PEBISNIS',
